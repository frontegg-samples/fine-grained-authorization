...
async function promptVendorCreds(): Promise<{ clientId: string; secret: string }> {
	...
	const clientIdAns = await prompt({type: "input", name: "clientId", message: "Enter your Frontegg environment Client ID"});
	const apiKeyAns = await prompt({type: "password", name: "apiKey", message: "Enter your Frontegg environment API Key"});
